...
template<typename T>
mkl_svd_t<T>::mkl_svd_t() {
    this->set_num_threads(DEFAULT_NUM_THREADS);
}

template<typename T>
mkl_svd_t<T>::mkl_svd_t(size_t num_threads) {
...
template<typename T>
void mkl_svd_t<T>::set_num_threads(size_t num_threads) {
...
template<>
void mkl_svd_t<float>::calculate(
    ...
    mkl_set_num_threads(this->num_threads);
